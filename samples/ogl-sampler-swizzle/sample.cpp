...
namespace
{
	std::string const SAMPLE_NAME = "OpenGL sampler swizzle";	
...
void sample::render()
{
	...
	for(std::size_t Index = 0; Index < viewport::MAX; ++Index)
	{
		...
		glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_SWIZZLE_R, this->SwizzleR[Index]);
		glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_SWIZZLE_G, this->SwizzleG[Index]);
		glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_SWIZZLE_B, this->SwizzleB[Index]);
		glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_SWIZZLE_A, this->SwizzleA[Index]);
...
bool sample::initTexture2D()
{
	...
	this->SwizzleR[viewport::V00] = GL_RED;
	this->SwizzleG[viewport::V00] = GL_GREEN;
	this->SwizzleB[viewport::V00] = GL_BLUE;
	this->SwizzleA[viewport::V00] = GL_ALPHA;

	this->SwizzleR[viewport::V10] = GL_BLUE;
	this->SwizzleG[viewport::V10] = GL_GREEN;
	this->SwizzleB[viewport::V10] = GL_RED;
	this->SwizzleA[viewport::V10] = GL_ALPHA;

	this->SwizzleR[viewport::V11] = GL_ONE;
	this->SwizzleG[viewport::V11] = GL_GREEN;
	this->SwizzleB[viewport::V11] = GL_BLUE;
	this->SwizzleA[viewport::V11] = GL_ALPHA;

	this->SwizzleR[viewport::V01] = GL_ZERO;
	this->SwizzleG[viewport::V01] = GL_GREEN;
	this->SwizzleB[viewport::V01] = GL_BLUE;
	this->SwizzleA[viewport::V01] = GL_ALPHA;
