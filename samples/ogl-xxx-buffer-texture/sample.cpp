...
namespace
{
	...
	std::string const VERTEX_SHADER_SOURCE(glf::DATA_DIRECTORY + "xxx/texture-buffer.vert");
	std::string const FRAGMENT_SHADER_SOURCE(glf::DATA_DIRECTORY + "xxx/texture-buffer.frag");
...
bool initTextureBuffer()
{
    ...
    glNamedBufferDataEXT(DisplacementBufferName, sizeof(Position), Position, GL_STATIC_DRAW);

	glGenTextures(1, &DisplacementTextureName);
	glTextureBufferEXT(DisplacementTextureName, GL_TEXTURE_BUFFER, GL_RGBA32F, DisplacementBufferName);
	...
	glNamedBufferDataEXT(GL_TEXTURE_BUFFER, sizeof(Diffuse), Diffuse, GL_STATIC_DRAW);

	glGenTextures(1, &DiffuseTextureName);
	glTextureBufferEXT(DiffuseTextureName, GL_TEXTURE_BUFFER, GL_RGBA8, DiffuseTextureName);
...
void display()
{
	...
	glProgramUniform1iEXT(ProgramName, UniformDisplacement, 0);
	glProgramUniform1iEXT(ProgramName, UniformDiffuse, 1);

	glBindMultiTextureEXT(GL_TEXTURE0, GL_TEXTURE_BUFFER, DisplacementTextureName);
	glBindMultiTextureEXT(GL_TEXTURE1, GL_TEXTURE_BUFFER, DiffuseTextureName);
